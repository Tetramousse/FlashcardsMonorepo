...
- Nginx (Reverse proxy, Rate Limiting, connessioni concorrenti) 
- Librerie core: pydantic, httpx, python-multipart, firebase-admin 
- Integrazioni: MarkItDown (Microsoft), Unstructured API, LLM Provider (OpenAI-compatible, es. Groq) 

---

## 🔌 API Endpoints (Backend)

[cite_start]Tutti gli endpoint richiedono autenticazione Firebase JWT nell'header `Authorization: Bearer <token>`. Ogni file è associato allo `user_id` Firebase e gli utenti possono operare solo sui propri file.
...
| `DELETE` | `/api/v1/files/{file_id}` | Elimina file e relativi chunk. |
| `POST` | `/api/v1/files/{file_id}/flashcards` | Genera flashcard AI (limite default 10) da chunk random del file specificato. |

---

## Rete, Rate Limit e Gateway (Nginx)

Il traffico API passa da Nginx, che applica le seguenti policy globali per IP (`location /`):

- **Rate limit**: 10 richieste/secondo (con burst fino a 20 richieste extra senza ritardo).
- **Connessioni concorrenti**: Massimo 10 connessioni per IP.
- **Comportamento Limiti**: Al superamento, Nginx risponde con `503 Service Temporarily Unavailable`.
- **Compressione**: Gzip attivo per JSON, testo puro, JS e CSS (lunghezza minima 1000).
...
- **Backend**: Scarica `serviceAccountKey.json` dalla Console Firebase (Impostazioni progetto → Account di servizio → Genera nuova chiave privata) e posizionalo nella root del backend (accanto a `main.py`).
- **Frontend**: Ottieni le credenziali client per configurare l'ambiente Angular.

### 2. Configurazione e Avvio Backend (API)
L'API è esposta su `http://localhost:9090`.

1. Naviga nella cartella: `cd backend`
2. Crea il file `.env` compilando i seguenti parametri:
   ```env
